...
export const BigStat = ({ children, color = C.cream, size = 300 }: any) => (
  <div style={{ fontFamily: PLAYFAIR, fontStyle: "italic", fontWeight: 500, fontSize: size, lineHeight: 0.82, letterSpacing: "-0.04em", color, marginBottom: 46 }}>
    {children}
  </div>
);
...
export const Handle = ({ children = "@lovehappinessfreedom" }: any) => (
  <div style={{ display: "inline-block", marginTop: 64, padding: "24px 40px", borderRadius: 999, background: C.cream, color: C.ink, fontFamily: MONO, fontSize: 24, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 500 }}>
    {children}
  </div>
);
...
// Motion scene for TransitionSeries reels (Drop 03): text rises + fades in,
// then fades OUT just before the image crossfade begins, so type never
// double-exposes across a blend. Backgrounds ride the blend at full opacity.
export const MScene = ({ children, align = "bottom", dur = 120, out = true }: any) => {
  const f = useCurrentFrame();
  const op = out
  ...
  const y = interpolate(f, [6, 28], [46, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.out(Easing.cubic) });
  const bottom = align === "bottom";
  return (
    <AbsoluteFill style={{ justifyContent: bottom ? "flex-end" : "center", padding: bottom ? "0 80px 170px" : "0 80px", opacity: op, transform: `translateY(${y}px)` }}>
      {children}
    </AbsoluteFill>
  );
};
